...
function preload() {
  ...
  brownbutton = loadImage('brownbutton.jpg');
...
function setup() {
 createCanvas(windowWidth,windowHeight);
  ...
  play_back_button = createSprite(width/1.1,height/10,20,20);
  play_back_button.addImage(bluebutton);
  play_back_button.scale = 0.2
  play_back_button.visible = false;

  INSTRUCTION_BUTTON = createSprite(width/1.2,height/10,20,20);
...
function draw() {
  ...
  if(GAMESTATE === INSTRUCTIONS) {
    ...
    text("JUMP PRESS J. YOU ARE GIVEN 10 LIFE AND WHEN YOU DONT HAVE ANY LIFE,",width/8.8,height/2.2);
    text("THE GAME IS OVER. DON'T FORGET THAT ENEMIES CAN ALSO SHOOT",width/8.8,height/1.950);
    text("THANK YOU-",width/8.8,height/1.5);
    textSize(50)
    text("INSTRUCTIONS",width/2.8,height/5);
    text("______________",width/2.8,height/4.8);
